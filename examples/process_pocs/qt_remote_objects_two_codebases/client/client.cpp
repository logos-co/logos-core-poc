...
public:
    CounterWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
        ...
        resize(400, 550);  // Increased height for message list
        ...
        // Add say hello button for ModuleCommunication
        QPushButton *sayHelloButton = new QPushButton("Say Hello", this);
        sayHelloButton->setMinimumSize(100, 40);
        sayHelloButton->setFont(buttonFont);
        
        resultLabel = new QLabel("Multiplication result: -", this);
        resultLabel->setAlignment(Qt::AlignCenter);
        
        // Add section for server messages
        QLabel *messagesTitle = new QLabel("Messages from Server:", this);
        messagesTitle->setAlignment(Qt::AlignLeft);
        messagesTitle->setFont(titleFont);
        
        // List widget for server messages
        messageList = new QListWidget(this);
        messageList->setMinimumHeight(100);
        ...
        mainLayout->addWidget(sayHelloButton);
        mainLayout->addWidget(resultLabel);
        mainLayout->addWidget(messagesTitle);
        ...
        // Get the ModuleCommunication from the registry
        moduleCommunication = remoteNode->acquireDynamic("ModuleCommunication");
        ...
        // Wait for ModuleCommunication to be valid, then connect to the messageReceived signal
        connect(moduleCommunication, &QRemoteObjectDynamicReplica::initialized, this, [this]() {
            // Connect to the dynamic messageReceived signal using string-based connection
            bool connected = connect(moduleCommunication, SIGNAL(messageReceived(QString)), 
                                   this, SLOT(handleServerMessage(QString)));
            if (!connected) {
        ...
        connect(sayHelloButton, &QPushButton::clicked, this, &CounterWindow::sayHello);
...
private slots:
    ...
    void sayHello() {
        if (moduleCommunication && moduleCommunication->state() == QRemoteObjectReplica::Valid) {
            qDebug() << "Calling sayHello on ModuleCommunication";
            QMetaObject::invokeMethod(moduleCommunication, "sayHello");
        } else {
            qDebug() << "ModuleCommunication is not valid. State:" 
                     << (moduleCommunication ? 
                         static_cast<int>(moduleCommunication->state()) : -1);
        }
    }
    ...
    void handleServerMessage(const QString &message) {
        ...
        // Add the message to the list widget with current time
        QString timeStr = QTime::currentTime().toString("hh:mm:ss");
        messageList->insertItem(0, QString("[%1] %2").arg(timeStr).arg(message));
        
        // Limit the number of messages displayed
        while (messageList->count() > 20) {
            delete messageList->takeItem(messageList->count() - 1);
        }
    }
    ...
    void multiplyCounter() {
        if (counter && counter->state() == QRemoteObjectReplica::Valid) {
            ...
            if (ok) {
                ...
                // Invoke the remote method with request ID
...
private:
    ...
    QRemoteObjectDynamicReplica *moduleCommunication;
