...
public:
    ...
    // New abstract operation method
    void executeOperationWithCallback(const QString& requestId, const QString& operation, const QVariantList& arguments) {
        qDebug() << "CounterProxy: Forwarding executeOperationWithCallback() to remote object"
                 << "Operation:" << operation << "Arguments:" << arguments;
        QMetaObject::invokeMethod(m_replica, "executeOperationWithCallback", 
                                  Qt::DirectConnection,
                                  Q_ARG(QString, requestId),
...
RemoteCounter::RemoteCounter(QObject* parent) : QObject(parent) {
    ...
    connect(m_counter, &QRemoteObjectDynamicReplica::initialized, this, [this, instanceId]() {
        ...
        // Connect to the new operationResult signal from the remote object
        connect(m_counter, SIGNAL(operationResult(QString,QVariantList)), 
                this, SLOT(onOperationResult(QString,QVariantList)));
...
void RemoteCounter::multiply(int factor, std::function<void(int)> callback) {
    qDebug() << "\n\n\n================ RemoteCounter::multiply - Factor:" << factor;
    if (!isConnected()) {
...
void RemoteCounter::onOperationResult(const QString& requestId, const QVariantList& results) {
    // Execute the callback for this request using the abstract callback manager
    qDebug() << "RemoteCounter::onOperationResult - Request ID:" << requestId << "Results:" << results;
    m_abstractCallbackManager.executeCallback(requestId, results);
}

void RemoteCounter::executeOperation(const QString& operation, const QVariantList& arguments, 
                                   std::function<void(const QVariantList&)> callback) {
    qDebug() << "RemoteCounter::executeOperation - Operation:" << operation << "Arguments:" << arguments;
    if (!isConnected()) {
        qDebug() << "Cannot execute operation: not connected";
        return;
    }
    
    // Register the callback and get a request ID
    QString requestId = m_abstractCallbackManager.registerCallback(callback);
