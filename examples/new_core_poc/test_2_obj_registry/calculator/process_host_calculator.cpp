...
// Custom object registry class
class CustomObjectRegistry : public QObject
{
    Q_OBJECT

public:
    CustomObjectRegistry(QObject *parent = nullptr) : QObject(parent) {
        // Empty for now
    }
    
    void addObject(const QString& name, QObject* object) {
        qDebug() << "Adding object to registry:" << name;
        if (object) {
            m_objects[name] = object;
            qDebug() << "Added object to registry:" << name;
        }
    }

public slots:
    Q_INVOKABLE QObject* getObject(const QString& name) {
        qDebug() << "Getting object from registry:" << name;
        if (m_objects.contains(name)) {
            qDebug() << "Found object in registry:" << name;
            return m_objects[name];
        }
        qDebug() << "Object not found in registry:" << name;
        return nullptr;
    }
...
int main(int argc, char *argv[])
{
    ...
    QRemoteObjectRegistryHost srcNode(QUrl(QStringLiteral("local:registry3")));

    // Create and share the counter
    Calculator *calculator = new Calculator();
    // srcNode.enableRemoting(calculator, "Calculator");

    // initialize custom object registry
    // Create custom object registry instance
    CustomObjectRegistry *customRegistry = new CustomObjectRegistry(&app);

    // Add calculator to custom registry
    customRegistry->addObject("Calculator", calculator);

    srcNode.enableRemoting(customRegistry, "CustomObjectRegistry");
