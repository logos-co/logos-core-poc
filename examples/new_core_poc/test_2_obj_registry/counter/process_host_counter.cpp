...
int main(int argc, char *argv[])
{
    ...
    QRemoteObjectNode *remoteNode = new QRemoteObjectNode(&app);
    remoteNode->connectToNode(QUrl(QStringLiteral("local:registry3")));

        // Get CustomObjectRegistry
    // QRemoteObjectDynamicReplica *customRegistryReplica = remoteNode->acquireDynamic("CustomObjectRegistry");
    QSharedPointer<QRemoteObjectDynamicReplica> customRegistryReplica(remoteNode->acquireDynamic("CustomObjectRegistry"));
    

    QObject::connect(customRegistryReplica.data(), &QRemoteObjectDynamicReplica::initialized, [=]() {
        qDebug() << "CustomObjectRegistry replica initialized, calling getObject";
        
        // Note: QObject* cannot be serialized across process boundaries in Qt Remote Objects
        // This call will likely fail with serialization error
        QObject* result = nullptr;
        bool success = QMetaObject::invokeMethod(customRegistryReplica.data(), "getObject", 
                                                Qt::DirectConnection,
                                                Q_RETURN_ARG(QObject*, result),
                                                Q_ARG(QString, "Calculator"));
        
        if (success) {
            qDebug() << "getObject call succeeded, result:" << result;
        } else {
            qDebug() << "getObject call failed";
        }
    });
    
    //// Wait for custom registry replica to be valid and call getObject
    //if (customRegistryReplica->waitForSource(1000)) {
    //    qDebug() << "CustomObjectRegistry replica is valid, calling getObject";
    //    
    //    // Call getObject("Calculator")
    //    QRemoteObjectPendingCall pendingCall;
    //    QMetaObject::invokeMethod(customRegistryReplica, "getObject", 
    //        Q_RETURN_ARG(QRemoteObjectPendingCall, pendingCall),
    //        Q_ARG(QString, "Calculator"));
    //        
    ...
    //    if (pendingCall.error() == QRemoteObjectPendingCall::NoError) {
    //        QVariant result = pendingCall.returnValue();
    //        qDebug() << "getObject result:" << result;
    //    } else {
    //        qDebug() << "getObject call failed with error:" << pendingCall.error();
    //    }
    //} else {
    //    qDebug() << "Failed to connect to CustomObjectRegistry";
    //}
    
    // // Calculator *calculator = remoteNode->acquireDynamic("Calculator");
    // // make it more abstract
    // QRemoteObjectDynamicReplica *calculatorReplica2 = remoteNode->acquireDynamic("Calculator");
    // // invoke add
    // // Wait for calculator replica to be valid
    // if (calculatorReplica2->waitForSource(1000)) {
    //     // Use QRemoteObjectPendingCall for dynamic replica method calls
    //     QVariantList args;
    //     args << 5 << 3;
    //     
    //     // For Qt Remote Objects, we need to get the pending call and wait for it
    //     QRemoteObjectPendingCall pendingCall;
    //     QMetaObject::invokeMethod(calculatorReplica2, "add", 
    //         Q_RETURN_ARG(QRemoteObjectPendingCall, pendingCall),
    //         Q_ARG(int, args[0].toInt()),
    //         Q_ARG(int, args[1].toInt()));
    //         
    //     // Wait for the call to complete
    //     pendingCall.waitForFinished();
    //     
    ...
    //         qDebug() << "Calculator add result:" << result;
    //     } else {
    //         qDebug() << "Calculator call failed with error:" << pendingCall.error();
    //     }
    //     
    //     // then exit
    //     // QCoreApplication::quit();
    // } else {
    //     qDebug() << "Failed to connect to calculator service";
    // }
