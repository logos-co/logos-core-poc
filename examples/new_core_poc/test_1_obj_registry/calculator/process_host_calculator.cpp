...
int main(int argc, char *argv[])
{
    ...
    // define qtremote object registry host
    // QRemoteObjectRegistryHost registryHost(QUrl("local:registry2"));
   
    // Create a registry host with a local URL (internal to this process only)
    QRemoteObjectRegistryHost srcNode(QUrl(QStringLiteral("")));

    // Create and share the counter
    Calculator *calculator = new Calculator(&srcNode);
    srcNode.enableRemoting(calculator, "Calculator");

    // get the object registry used by srcNode
    const QRemoteObjectRegistry *objectRegistry = srcNode.registry();
    qDebug() << "Object registry:" << objectRegistry;

    // Query the registry to get information about registered objects
    if (objectRegistry) {
        qDebug() << "Registry is available, querying for objects...";
        
        // Get the source locations (URLs) of all registered objects
        QRemoteObjectSourceLocations sourceLocations = objectRegistry->sourceLocations();
        qDebug() << "Found" << sourceLocations.size() << "registered objects:";
        
        // Iterate through all registered objects
        for (auto it = sourceLocations.begin(); it != sourceLocations.end(); ++it) {
            QString objectName = it.key();
            QRemoteObjectSourceLocationInfo locationInfo = it.value();
            
            qDebug() << "  Object name:" << objectName;
            qDebug() << "  Host URL:" << locationInfo.hostUrl;
            qDebug() << "  Type name:" << locationInfo.typeName;
        }
        
        // Check if Calculator is registered
        if (sourceLocations.contains("Calculator")) {
            qDebug() << "Calculator found in registry!";
            QRemoteObjectSourceLocationInfo calcInfo = sourceLocations["Calculator"];
            qDebug() << "Calculator host URL:" << calcInfo.hostUrl;
            qDebug() << "Calculator type:" << calcInfo.typeName;
            
            // You can now connect to the calculator using this information
            // For example, create a node and acquire the calculator replica
            QRemoteObjectNode *clientNode = new QRemoteObjectNode(&app);
            clientNode->connectToNode(calcInfo.hostUrl);
            
            // Acquire the calculator replica
            QRemoteObjectDynamicReplica *calculatorReplica = clientNode->acquireDynamic("Calculator");
            
            //if (calculatorReplica->waitForSource(1000)) {
            //    qDebug() << "Successfully connected to Calculator via registry!";
            //    
            //    // Test the calculator
            //    QVariantList args;
            //    args << 10 << 5;
            //    
            //    QRemoteObjectPendingCall pendingCall = calculatorReplica->call("add", args);
            //    pendingCall.waitForFinished();
            //    
            //    if (pendingCall.error() == QRemoteObjectPendingCall::NoError) {
            //        int result = pendingCall.returnValue().toInt();
            //        qDebug() << "Calculator add result via registry:" << result;
            //    } else {
            //        qDebug() << "Calculator call failed:" << pendingCall.error();
            //    }
            //} else {
            //    qDebug() << "Failed to connect to Calculator";
            //}
        } else {
            qDebug() << "Calculator not found in registry";
        }
    } else {
        qDebug() << "Registry is not available";
    }
